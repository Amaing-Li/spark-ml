...
object RandomForestRegression {
  def main(args: Array[String]) {
    ...
    val rfModel = new RandomForestRegressor().setLabelCol("indexedLabel").setFeaturesCol("indexedFeatures").setNumTrees(100).fit(trainFeatureIndexed)


    val test: DataFrame = spark.read.format("csv").load("/random/test.csv")

    //句子分成单词
    val testTokenized1: DataFrame =  new Tokenizer().setInputCol("_c1").setOutputCol("words1").transform(test)
    ...
    predictions.select("_c0","prediction")
